...
| macOS 10.11             |  amd64   | [![macOS 10.11 amd64](https://build.txchange.online/png?builder=monero-static-osx-10.11)](https://build.txchange.online/builders/monero-static-osx-10.11)
| macOS 10.12             |  amd64   | [![macOS 10.12 amd64](https://build.txchange.online/png?builder=monero-static-osx-10.12)](https://build.txchange.online/builders/monero-static-osx-10.12)
| macOS 10.13             |  amd64   | [![macOS 10.13 amd64](https://build.txchange.online/png?builder=monero-static-osx-10.13)](https://build.txchange.online/builders/monero-static-osx-10.13)
| FreeBSD 11            |  amd64   | [![FreeBSD 11 amd64](https://build.txchange.online/png?builder=monero-static-freebsd64)](https://build.txchange.online/builders/monero-static-freebsd64)
| DragonFly BSD 4.6     |  amd64   | [![DragonFly BSD amd64](https://build.txchange.online/png?builder=monero-static-dragonflybsd-amd64)](https://build.txchange.online/builders/monero-static-dragonflybsd-amd64)
| Windows (MSYS2/MinGW) |  i686    | [![Windows (MSYS2/MinGW) i686](https://build.txchange.online/png?builder=monero-static-win32)](https://build.txchange.online/builders/monero-static-win32)
| Windows (MSYS2/MinGW) |  amd64   | [![Windows (MSYS2/MinGW) amd64](https://build.txchange.online/png?builder=monero-static-win64)](https://build.txchange.online/builders/monero-static-win64)
...
| Coveralls | [![Coveralls Status](https://coveralls.io/repos/github/txchangecoin-project/monero/badge.svg?branch=master)](https://coveralls.io/github/txchangecoin-project/monero?branch=master)
| License   | [![License](https://img.shields.io/badge/license-BSD3-blue.svg)](https://opensource.org/licenses/BSD-3-Clause)

## Introduction

Txchangecoin is a private, secure, untraceable, decentralised digital currency. You are your bank, you control your funds, and nobody can trace your transfers unless you allow them to do so.

**Privacy:** Txchangecoin uses a cryptographically sound system to allow you to send and receive funds without your transactions being easily revealed on the blockchain (the ledger of transactions that everyone has). This ensures that your purchases, receipts, and all transfers remain absolutely private by default.
...
Monero is a 100% community-sponsored endeavor. If you want to join our efforts, the easiest thing you can do is support the project financially. Both Monero and Bitcoin donations can be made to **donate.txchange.online** if using a client that supports the [OpenAlias](https://openalias.org) standard. Alternatively you can send XMR to the Monero donation address via the `donate` command (type `help` in the command-line wallet for details).
...
`$ cd txchangecoin && git submodule init && git submodule update`
...
* Run Monero with `txchangecoind --detach`
...
```
        git clone https://github.com/txchangecoin-project/txchangecoin.git
	cd txchangecoin
	git checkout tags/v0.1.1.0
```
* Build:
```
        make release
...
* To git clone, run:
        ...
        git clone --recursive https://github.com/txchangecoin-project/txchangecoin.git
...
* If you would like a specific [version/tag](https://github.com/txchangecoin-project/txchangecoin/tags), do a git checkout for that version. eg. 'v0.1.1.0'. If you don't care about the version and just want binaries from master, skip this step:
	
        git checkout v0.1.1.0
...
* Docker
        ...
        docker run -it -v /monero/chain:/root/.txchangecoin -v /monero/wallet:/wallet -p 38380:38380 monero

        # or in background
        docker run -it -d -v /monero/chain:/root/.txchangecoin -v /monero/wallet:/wallet -p 38380:38380 txchangecoin
...
To list all available options, run `./bin/txchangecoind --help`.  Options can be
...
To run in background:
    ...
    ./bin/txchangecoind --log-file txchangecoind.log --detach

To run as a systemd service, copy
[txchangecoind.service](utils/systemd/txchangecoind.service) to `/etc/systemd/system/` and
[txchangecoind.conf](utils/conf/txchangecoind.conf) to `/etc/`. The [example
service](utils/systemd/txchangecoind.service) assumes that the user `txchangecoin` exists
and its home is the data directory specified in the [example
config](utils/conf/txchangecoind.conf).

If you're on Mac, you may need to add the `--max-concurrency 1` option to
txchangecoin-wallet-cli, and possibly txchangecoind, if you get crashes refreshing.
...
While txchangecoin isn't made to integrate with Tor, it can be used wrapped with torsocks, by
setting the following configuration parameters and environment variables:

* `--p2p-bind-ip 127.0.0.1` on the command line or `p2p-bind-ip=127.0.0.1` in
  txchangecoind.conf to disable listening for connections on external interfaces.
* `--no-igd` on the command line or `no-igd=1` in txchangecoind.conf to disable IGD
  (UPnP port forwarding negotiation), which is pointless with Tor.
* `DNS_PUBLIC=tcp` or `DNS_PUBLIC=tcp://x.x.x.x` where x.x.x.x is the IP of the
  desired DNS server, for DNS requests to go over TCP, so that they are routed
  through Tor. When IP is not specified, txchangecoind uses the default list of
  servers defined in [src/common/dns_utils.cpp](src/common/dns_utils.cpp).
* `TORSOCKS_ALLOW_INBOUND=1` to tell torsocks to allow txchangecoind to bind to interfaces
...
* Do NOT pass `--detach` when running through torsocks with systemd, (see
  [utils/systemd/txchangecoind.service](utils/systemd/txchangecoind.service) for details).
* If you use the wallet with a Tor daemon via the loopback IP (eg, 127.0.0.1:9050),
  then use `--untrusted-daemon` unless it is your own hidden service.

Example command line to start txchangecoind through Tor:

    DNS_PUBLIC=tcp torsocks txchangecoind --p2p-bind-ip 127.0.0.1 --no-igd
...
allow inbound connections. Full example:
    ...
    sudo iptables -I OUTPUT 2 -p tcp -d 127.0.0.1 -m tcp --dport 38381 -j ACCEPT
    DNS_PUBLIC=tcp torsocks ./txchangecoind --p2p-bind-ip 127.0.0.1 --no-igd --rpc-bind-ip 127.0.0.1 \
        --data-dir /home/amnesia/Persistent/your/directory/to/the/blockchain

## Debugging

This section contains general instructions for debugging failed installs or problems encountered with txchangecoin. First, ensure you are running the latest version built from the Github repo.
...
```
gdb /path/to/txchangecoind `pidof txchangecoind`
```
...
When it terminates with an output along the lines of "Segmentation fault (core dumped)", there should be a core dump file in the same directory as txchangecoind. It may be named just `core`, or `core.xxxx` with numbers appended.

You can now analyse this core dump with `gdb` as follows:

`gdb /path/to/txchangecoind /path/to/dumpfile`

Print the stack trace with `bt`

* To run txchangecoin within gdb:

Type `gdb /path/to/txchangecoind`

Pass command-line options with `--args` followed by the relevant arguments

Type `run` to run txchangecoind
...
Configure txchangecoin with the -D SANITIZE=ON cmake flag, eg:

    cd build/debug && cmake -D SANITIZE=ON -D CMAKE_BUILD_TYPE=Debug ../..

You can then run the txchangecoin tools normally. Performance will typically halve.

* valgrind

Install valgrind and run as `valgrind /path/to/txchangecoind`. It will be very slow.
...
`cd ~/txchangecoin/external/db_drivers/liblmdb && make`
